...
## 5. 에러와 해결방법
  - 가장 많이 에러가 났던 부분은 단연 라우터... 아직 미숙한 부분인만큼 프로젝트 진행 중에도 구글링을 통해 지속 학습하고, 내가 적은 코드를 되풀이해보면서 문제점이 무엇인지를 짚어나가고, 꼬여버린 코드들을 하나씩 다시 풀어나가기를 반복했다.  
아직 완벽하게 숙지했다. 라고 하기는 어려운 수준이지만, 추후 지속적인 develop을 통해 한층 더 발전해나갈 수 있지않을까. 가장 많이 수정했던 부분은 중복 초기화/랜더링, 그리고 가장 애먹었던 것은 코드 순서.
