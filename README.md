...
에러를 해결하는것에 가장 많이 도움이 되었던 것은 아무래도 익숙치않은 코드들이다보니, 내가 작성한 코드들을 다시 한줄한줄 훑어보며 어떻게 동작하는지 눈에 익히고, 살펴보는것이 아니었을까 생각한다.  
  -  새로고침시 hash(주소)는 보고있던 화면이나, 랜딩되는 화면은 Main(상품목록페이지)인 것에대한 오류 : index.js에 window에 load event를 부여함으로써, 변경된 페이지를 sessionStorage에 저장하고, 기억하여 페이지 로드시에 저장된 페이지가 로드될 수 있도록 하여 에러를 해결하였다.
